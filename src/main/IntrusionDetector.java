...
public class IntrusionDetector extends JFrame {
    ...
    public void automateDetections(){
        //Machine Learning aspect of IntrusionDetector Program ( AI )
        NeuralNetwork model = null;
        double numberOfEpochs = 0;
        int batchSize = 0;
        LossFunction lossFunction = null;
        LearningRate learningRate = null;

        NetworkProcessor processor = new NetworkProcessor(model,main,numberOfEpochs,batchSize,lossFunction,learningRate);
        processor.trainModel();
        processor.runFinalModel();
    }
...
/*
* Once requestMap is properly built where usernames point to a list of all the requests they have made then we can continue to
